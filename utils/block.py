...
def unbalanced_block_model(nb_of_clust, clust_size_min, clust_size_max, p, q):
    ...
    for r in range(nb_of_clust):
        ...
        c.append(val_r)
    c = np.concatenate(c) # Cluster label (1-dim)
    W = block_model(c,p,q) # V x V
    return W,c
...
def add_a_block(W0,W,c,nb_of_clust,q):
    ...
    W_up=np.concatenate(  ( W , V.T ) , axis=1 ) # n x (n+n0)
    W_low=np.concatenate( ( V , W0  ) , axis=1 ) # n0 x (n+n0)
    W_new=np.concatenate( (W_up,W_low)  , axis=0) # (n+n0) x (n+n0)
